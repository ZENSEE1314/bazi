    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-ink/10 bg-white p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <Link to={`/profiles/${profile.id}`} className="btn-ghost text-sm">{t("dash.open_full")}</Link>
        </div>

        {bazi && (
          <>
            <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
              <PillarCard label={t("dash.year")} pillar={bazi.year} />
              <PillarCard label={t("dash.month")} pillar={bazi.month} />
              <PillarCard label={t("dash.day")} pillar={bazi.day} />
              <PillarCard label={t("dash.hour")} pillar={bazi.hour} />
            </div>
            <div className="mt-4 text-sm">
              {t("dash.day_master")}: <b>{bazi.day_master}</b> ({bazi.day_master_element}) • {t("dash.zodiac")}: <b>{bazi.zodiac}</b> • {t("dash.dominant")}: <b>{bazi.dominant_element}</b> • {t("dash.weakest")}: <b>{bazi.weakest_element}</b>
            </div>
            <div className="mt-3">
      ...
      {daily && (
        <section className="rounded-2xl border border-ink/10 bg-white p-5">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="text-xs text-muted uppercase tracking-wider">{t("dash.todays_luck")} · {daily.date}</div>
          ...
          {daily.supportive_elements.length > 0 && (
            <div className="mt-3">
              <div className="text-xs uppercase tracking-wider text-muted mb-1">{t("dash.supportive")}</div>
          ...
          {daily.clashing_elements.length > 0 && (
            <div className="mt-3">
              <div className="text-xs uppercase tracking-wider text-muted mb-1">{t("dash.friction")}</div>
              <ul className="list-disc pl-5 text-sm space-y-0.5">
